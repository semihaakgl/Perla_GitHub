...
window.addEventListener("load", () => {     //? sayfa ilk açıldığında demek.
    localStorage.setItem("taxRate", taxRate);   //? setItem göndermek anlamındadır
...
const productsDiv = document.querySelector(".products"); //? en kapsayıcı div i seçtik.
...
productsDiv.addEventListener("click", (e) => {
    ...
    if (e.target.className === "fa-solid fa-minus") {
        ...
        if (e.target.nextElementSibling.innerText > 1) {  //? target tıkladığın yeri yakala anlamındadır.
            e.target.nextElementSibling.innerText--;  //? innertext içine anlamındadır.
            ...
            if (confirm(`${e.target.closest(".product-info").querySelector("h2").innerText} will be removed!`)) {
                e.target.closest(".product").remove();  //? remove sil demek.
            }
        }
        calculateCartPrice();
    }
        ...
        alert("other element clicked");  //? herhangi bir yere tıklandığında alert ile uyarı gitsin anlamındadır.
    }
...
const calculateProductPrice = (target) => {
    ...
    const productInfoDiv = target.closest(".product-info");  //? closest en yakın perınt.
...
const calculateCartPrice = () => {
    ...
    const taxPrice = subtotal * localStorage.getItem("taxRate");  //? getıtem almak anlamındadır.
